...
class Pet:
    def __init__(self, signo, nome, especie, idade, altura, peso, nome_dono, vacinado, hospedado=False):
        self.signo = signo
        self.nome = nome
    ...
    def criar_de_dicionario(dados):
        ...
        Usa .get() para evitar KeyError caso o JSON tenha dados antigos.
        """
        return Pet(
            signo=dados.get("signo", "Não informado"),
            nome=dados.get("nome", "Indefinido"),
            especie=dados.get("especie", "Não informada"),
            idade=dados.get("idade", 0),
            altura=dados.get("altura", 0.0),  # Se não existir, assume 0.0
            peso=dados.get("peso", 0.0),
            nome_dono=dados.get("nome_dono", "Sem dono"),
            vacinado=dados.get("vacinado", False),
...
def cadastrar_pet(lista_pets):
    ...
    pet = Pet(signo, nome, especie, idade, altura, peso, nome_dono, vacinado)
    lista_pets.append(pet)
